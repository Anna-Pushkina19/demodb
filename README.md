...
Некоторые внутренние настройки не вынесены на уровень пользователя: присутствуют в исходном коде в виде постоянных функций (неудачные значения могут привести к ошибкам генерации или к неожиданным результатам) или только в виде констант.
...
Язык, на котором будут выводиться названия моделей самолетов, стран, городов и аэропортов. Задаётся при генерации. Доступны русский (`ru`) и английский (`en`) языки.
...
После прерывания генерации её можно начать заново с помощью `generate`. Использовать `continue` не рекомендуется, так как не гарантирует корректного продолжения.

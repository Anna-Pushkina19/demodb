...
* На уровень пользователя настройки не вынесены. Некоторые параметры присутствуют в исходном коде в виде постоянных функций. Неудачные значения могут привести к ошибкам генерации или к неожиданным результатам. Часть параметров присутствует в коде только в виде констант.
...
Обратите внимание, что чем меньше значение, тем больше рейсов доступно для формирования маршрута и выше риск опоздать на стыковочный рейс при задержке предыдущего.
...
Обратите внимание, что чем больше значение, тем больше рейсов доступно для формирования маршрута и длиннее время ожидания следующего рейса. Слишком большое значение будет неправдоподобным.
...
Обратите внимание, что чем больше значение, тем больше вероятность, что пассажир сумеет воспользоваться авиакомпанией, чтобы добраться до желаемого пункта.
...
После прерывания генерации её можно начать заново с помощью `generate` или использовать `continue`, что, однако, не гарантирует корректного продолжения.
...
* Чтобы добавить страну, добавьте имена в справочники генератора — `firstnames.dat` и `lastnames.dat` (за подробностями обратитесь к секции [«Вопросы и ответы»](#add-new-country).
...
Вердикт `ERROR: absent flights` означает, что в таблице `bookings.flights` отсутствуют рейсы, которые должны быть согласно таблице `routes`.
...
Подробнее о добавлении новых стран смотрите [ниже](#add-new-country).
...
### Как добавить новую страну? {#add-new-country}

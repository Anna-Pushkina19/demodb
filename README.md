...
1. Сделайте локальную копию репозитория `demodb`.
2. В psql подключитесь к любой базе данных, кроме `demo`.
Обратите внимание, что если база `demo` уже существует, то в ходе установки она будет удалена и все данные в ней будут потеряны!
3. Убедитесь, что вы находитесь в каталоге репозитория `demodb`.
...
Чтобы запустить генерацию, укажите начальное и конечное модельное время и выполните процедуру `generate`.
...
Обратите внимание, что разрыв соединения не завершает работу генератора. 
...
Если генерация успешно завершена, в выводе будет `f` (false).
...
Чтобы создать SQL-скрипт, в операционной системе в каталоге репозитория выполните команду:
...
1. Установка на уровне сеанса (`SET`) не сработает, так как доступ к значениям параметров должны иметь параллельные процессы.
2. На уровень пользователя настройки не вынесены. Некоторые параметры присутствуют в исходном коде в виде постоянных функций. Неудачные значения могут привести к ошибкам генерации или к неожиданным результатам. Часть параметров присутствует в коде только в виде констант.
...
Коэффициент пересчёта минут полёта в стоимость билета в предпочитаемой валюте. В зависимости от класса обслуживания применяется дополнительный повышающий коэффициент (функция `get_price`): билеты класса **Comfort** стоят в 1,3 раза дороже **Economy**, а **Business** — в два раза дороже **Economy**. 
...
Обратите внимание: чем меньше значение, тем больше рейсов доступно для формирования маршрута и выше риск опоздать на стыковочный рейс при задержке предыдущего.
...
Обратите внимание: чем больше значение, тем больше рейсов доступно для формирования маршрута и длиннее время ожидания следующего рейса. Слишком большое значение будет неправдоподобным.
...
Обратите внимание: чем больше значение, тем больше вероятность, что пассажир сумеет воспользоваться авиакомпанией, чтобы добраться до желаемого пункта.
...
Язык, на котором будут выводиться названия моделей самолетов, стран, городов и аэропортов. По умолчанию поддерживаются русский (`ru`) и английский (`en`) языки.
...
Обратите внимание: после прерывания генерации можно начать её заново с помощью `generate` или использовать `continue`, что, однако, не гарантирует корректного продолжения.
...
Пример вызова, в результате которого выводится десять случайных непальских имён:
...
1. Чтобы добавить страну, добавьте имена в справочники генератора — `firstnames.dat` и `lastnames.dat` (см. секцию "Вопросы и ответы" ниже).
2. Функция не требуется для работы с демобазой, но может быть полезной для наполнения других баз данных, в которых требуются случайные имена с правдоподобным распределением.
...
Ниже перечислены основные сообщения и их значения:
  ...
  * New bookings: _B_ (forceoneway _F_), nopath _P_, noseat _S_
  ...
  За модельный день было создано _B_ бронирований. Из них _F_ планировались как бронирования «туда и обратно» (с учётом целевого значения `gen.roundtrip_frac`), но из-за отсутствия нужных билетов бронь была сделана только в одну сторону. _P_ раз создать бронирование не удались из-за отсутствия подходящих рейсов, _S_ раз создать бронирование не удались из-за того, что на одном из выбранных рейсов не оказалось свободных мест.
...
1. В разные периоды времени один и тот же маршрут может обслуживаться разными самолетами, поэтому выведенные числа не стоит складывать. 
2. Вердикт `NOT USED` означает, что модель не выполнила ни одного рейса. Такую модель следует заменить. 
3. Вердикт `WRONGLY USED` свидетельствует об ошибке в алгоритме.

### Roundtrips to overall tickets

Отношение количества билетов «туда и обратно» (`roundtrip_frac`) к целевому значению (`target_frac`), заданному параметром `gen.roundtrip_frac`. Реальное значение всегда будет меньше целевого из-за того, что не для каждого прямого билета удаётся купить обратный.

### Passengers per booking

Первый запрос показывает, что в бронировании не было более `gen.max_pass_per_booking` пассажиров. Сообщение `ERROR: max_pass_per_bookings not satisfied` означает, что это правило нарушается, и свидетельствует об ошибке в алгоритме.
...
Обратите внимание, что независимо от размера базы в трёх статусах — `On Time` (вылет по расписанию), `Departed` (вылетел и находится в воздухе) и `Boarding` (посадка пассажиров) — рейсов будет немного. Если таких рейсов нет, продолжите генерацию на несколько модельных часов, чтобы обеспечить разнообразие данных.

### Flight durations

Вердикт `ERROR: route and flight discrepancy` в первом запросе говорит о рассинхронизации данных о продолжительности полётов между таблицами `routes` и `flights` и свидетельствует об ошибке в алгоритме.
...
Минимальная (`min_..._delay`), средняя (`avg_..._delay`) и максимальная (`max_..._delay`) задержка вылетов (`..._dep_...`) и прибытий (`..._arr_...`).
...
Вердикт `ERROR: non-adjacent segments` означает, что аэропорт прибытия одного перелёта не совпадает с аэропортом вылета следующего перелёта из того же билета. Это свидетельствует об ошибке в алгоритме.
...
Вердикт `ERROR: absent flights` означает, что в таблице `flights` отсутствуют рейсы, которые должны быть согласно таблице `routes`.

Вердикт `ERROR: excess flights` означает, что в таблице `flights` есть рейсы, не соответствующие расписанию в таблице `routes`.
...
Вердикт `ERROR: boarding after miss` во втором запросе означает, что пассажир садился в самолет после опоздания и свидетельствует об ошибке в алгоритме.
...
Демобаза содержит две схемы:
   ...
   * `gen` — с таблицами генераторы, которые используются при генерации и хранят её текущее состояние. 
...
Доступные таблицы из схемы `gen` описаны ниже.
...
##### Формирование графа маршрутов

Для формирования связанного графа маршрутов:

1. Берутся аэропорты с непустым значением в столбце `traffic` таблицы `airports_data`. 
...
3. Рёбра графа записываются в таблицу `directions`.
4. Рёбра добавляются пока граф не становится связным. Связность графа отслеживается с помощью таблицы `directions_connect`.
5. Новые маршруты переносятся из таблицы `directions` в таблицу `bookings.routes` с соответствующим интервалом действия (`validity`).
6. Самолёт, обслуживающий маршрут, и количество рейсов в неделю подбираются исходя из прогнозируемого трафика между аэропортами (таблица `week_traffic`).
...
2. Определяется маршрут до выбранного аэропорта (функция `get_path`) по следующим правилам:
   ...
   * Перелёт в выбранном маршруте начинается раньше.
...
Обратите внимание, если не удаётся найти подходящий маршрут или на выбранный маршрут не удаётся забронировать билеты на всех пассажиров, попытка бронирования считается неудачной и отменяется.
...
С вероятностью `gen.cancel_frac` рейс отменяется (статус `Cancelled`). Отмена всегда происходит заранее: рейс не отменяется уже после того, как на него куплены какие-либо билеты.

За день до отправления неотмененного рейса на него открывается регистрация, за которую отвечает добавляемое событие `REGISTRATION`. 
...
Обратите внимание, что пассажир проходит регистрацию только один раз, на первый рейс в билете.
...
Поэтому раз в неделю модельного времени очистка и анализ запускаются принудительно. Запуск происходит в отдельном процессе с помощью расширения `dblink`.
...
Таблица, содержащая отработанные события из таблицы `events` для возможности отладки.

### gen.airplanes\_data

Таблица со сгенерированными данными о моделях самолетов. Все строки и все столбцы, за исключением столбца `in_use`, который определяет, надо ли назначать данную модель на рейсы, переносятся в таблицу `bookings.airplanes_data`.

### gen.seats

Таблица со сгенерированными данными о конфигурации салонов самолётов. Все данные без изменений переносятся в таблицу `bookings.seats`.
...
Таблица со сгенерированными данными об аэропортах. В таблицу `bookings.airports_data` переносятся все строки и все столбцы, за исключением столбцов:
...
Журнальная таблица генератора. За подробностями обратитесь к разделу "Журнальная таблица".

## Вопросы и ответы

### Как добавить для демобазы новый язык?

Язык перевода в демобазе выбирается параметром `bookings.lang`. Переводятся названия моделей самолетов (представление `airplanes`), а также названия аэропортов, городов и стран (представление `airports`). В настоящий момент по умолчанию поддерживаются два языка: русский (`ru`) и английский (`en`).
...
Это непростая задача, поскольку в таблице аэропортов содержится около 5500 строк. Как минимум необходимо перевести строки с непустым значением пассажиропотока (`traffic`), то есть названия аэропортов, участвующих в формировании маршрута.
...
Подробнее о добавлении новых стран смотрите ниже.

### Каков критерий выбора количества аэропортов?

По умолчанию количество аэропортов в демобазе пропорционально квадрату суммы логарифмов площадей стран и численности их населения:

$$ 0,5 ( log( площадь, км^2 ) + log( население, млн ) - 5 )^2 $$

Подробнее о добавлении новых аэропортов смотрите ниже.
...
Чтобы добавить новый аэропорт, добавьте новую строку в таблицу `gen.airports_data` и заполните её следующей информацией:
   ...
   * код Международной ассоциации воздушного транспорта (airport_code);
...
Полной актуальной базы аэропортов в свободном доступе нет. Существует несколько открытых проектов (полнота и актуальность информации не гарантируется):
   * [OpenFlights](https://openflights.org/data.php)
   * [OurAirports](https://ourairports.com/data/)
   * [DataHub](https://datahub.io/core/airport-codes)), но полнота и актуальность их информации под вопросом. 

Конкретный аэропорт [можно проверить](https://www.iata.org/en/publications/directories/code-search/) на официальном сайте ИАТА.


Чтобы новая запись в таблице `gen.airports_data` была согласована с существующими, стоит придерживаться некоторых правил:

1. В качестве города (`city`) выбирается город, который обслуживается этим аэропортом. Часто это ближайший к аэропорту крупный населённый пункт, а не тот пункт, в котором расположен аэропорт. Например, для IAR городом является Ярославль, а не село Туношна.
2. Официальное название аэропорта часто состоит из названия населенного пункта, в котором расположен аэропорт, посвящения какому-либо видному деятелю и других частей. Для краткости имя сокращается по следующим правилам:
